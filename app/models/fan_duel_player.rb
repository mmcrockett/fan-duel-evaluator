...
class FanDuelPlayer < ActiveRecord::Base
  ...
  def fpoints
    ...
    self.game_data.each do |gdata|
      if (true == game_data.is_a?(GameData))
        gd = gdata
      else
  ...
  def self.load_player_details(params = {})
    ...
    if ((nil != import) && (nil != import.fd_contest_id))
      ...
      return_status[:total] = fd_players.size
      ...
      fd_players.each do |fd_player|
        if ((false == fd_player.game_data_loaded) || (DateTime.now > fd_player.updated_at + FanDuelPlayer::MINIMUM_UPDATE_TIME))
          ...
          begin
            ...
            if (false == exceptions.include?(e.message))
              e_message = "#{e}"
              exceptions[e.message] = 1
            else
              exceptions[e.message] += 1
            end
            logger.warn "Couldn't load player '#{fd_player.name}' in import '#{fd_player.import_id}' - '#{uri}'. #{e_message}"

            if ((exceptions[e.message] >= EXCEPTION_LIMIT) || (exceptions.keys.size >= EXCEPTION_LIMIT))
              logger.warn("Exceeded max number of exceptions. Breaking loop.")
              break
            end
          end
        else
    ...
    logger.warn("Done. Skipping '#{skipped_players.size}'. Saving '#{altered_players.size}'.")
    ...
    return_status[:status]  = exceptions.empty?
    return_status[:altered] = altered_players.size
    return_status[:skipped] = skipped_players.size
