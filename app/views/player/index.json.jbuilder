...
if (nil != week)
  ...
  FanDuelPlayer.where({:week => week}).each do |fd_player|
    ...
    player[:dvoa]     = number_with_precision(player[:avg] * Dvoa.adjustment(week, player[:position], player[:opponent]), :precision => 2)
    player[:fftoday]  = number_with_precision(player[:avg] * FfTodayPrediction.adjustment(week, player[:position], player[:team]), :precision => 2)
    players << player
  end
  ...
  players.each do |player|
    ...
    puts "STDDEVS: '#{player[:avg]}' '#{stats[position][:mean]}' '#{stats[position][:var]}' '#{Math.sqrt(stats[position][:var])}'"
    player[:stddevs] = number_with_precision((player[:avg] - stats[position][:mean])/Math.sqrt(stats[position][:var]), :precision => 2)
  end
end
