app.controller('WeekDataController', ['$scope', '$http', 'WeekData', 'FanDuelData', function($scope, $http, WeekData, FanDuelData) {
  ...
  $scope.add_week = function() {
    new WeekData({week:$scope.week_field}).$save({}, function(v){$scope.new_week = true;$scope.get_week_data()}, function(e){console.error("!ERROR adding week data.");});
  };
  $scope.add_fan_duel_json = function() {
    new FanDuelData({data:$scope.fan_duel_data}).$save({}, function(v){$scope.get_week_data()}, function(e){console.error("!ERROR saving fan duel json.");});
  };
