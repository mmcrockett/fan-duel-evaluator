...
class GameDataTest < ActiveSupport::TestCase
  ...
  test "if game data gets a GameData object in the constructor it returns that object" do
    game_data = JSON.parse(JSON_GAME_DATA)
    game_data_new_format = JSON.parse(JSON_GAME_DATA_NEW_FORMAT)

    game_data.concat(game_data_new_format).each do |game,i|
      game_data_model = GameData.new(game)
      game_data_model_copy = GameData.new(game_data_model)

      assert_equal(game_data_model.to_json, game_data_model_copy.to_json)
    end
  end

  test "if game data gets a non hash, it fails" do
    assert_raise Exception do |x|
      game_data_model = GameData.new("hello")
    end
  end

  test "to_s makes sense" do
    game_data_new_format = JSON.parse(JSON_GAME_DATA_NEW_FORMAT)

    game_data_new_format.each_with_index do |game,i|
      representation      = {:date => game_data_new_format[i]['date'], :data => game_data_new_format[i]['data']}
      game_data_model     = GameData.new(game)

      assert_equal("#{representation}", "#{game_data_model}")
      assert_equal(representation.to_json, game_data_model.to_json)
    end
  end
end
